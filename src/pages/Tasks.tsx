...
const priorityConfig: Record<TaskPriority, { style: string; icon: React.ReactNode }> = {
  ...
  low: {
    style: "bg-secondary/80 text-muted-foreground border-border/50",
...
const columnAccent: Record<TaskStatus, { header: string; dropzone: string; dot: string }> = {
  backlog: {
    header: "border-b-muted-foreground/30",
    dropzone: "border-muted-foreground/15",
    dot: "bg-muted-foreground",
  },
  todo: {
    header: "border-b-primary/40",
    dropzone: "border-primary/15",
    dot: "bg-primary",
  },
  "in-progress": {
    header: "border-b-warning/40",
    dropzone: "border-warning/15",
    dot: "bg-warning",
  },
  review: {
    header: "border-b-info/40",
    dropzone: "border-info/15",
    dot: "bg-info",
  },
  done: {
    header: "border-b-success/40",
    dropzone: "border-success/15",
    dot: "bg-success",
  },
};
          ...
          >
            <div className="glass-panel rounded-xl p-4 space-y-3 border border-border/60">
              <input
                ...
                autoFocus
                className="w-full bg-secondary/40 border border-border/40 rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20 transition-all"
              />
              <textarea
                ...
                rows={2}
                className="w-full bg-secondary/40 border border-border/40 rounded-lg px-3 py-2 text-sm text-foreground placeholder:text-muted-foreground/60 focus:outline-none focus:border-primary/50 focus:ring-1 focus:ring-primary/20 resize-none transition-all"
              />
              <div className="flex items-center gap-2">
                <select
                  value={newPriority}
                  onChange={(e) => setNewPriority(e.target.value as TaskPriority)}
                  className="bg-secondary/40 border border-border/40 rounded-lg px-3 py-1.5 text-xs text-foreground focus:outline-none focus:border-primary/50"
                >
          ...
          return (
            ...
            >
              ...
              <div className={`flex items-center gap-2 px-3 py-2 mb-2 border-b-2 ${accent.header}`}>
                ...
                <span className="text-[11px] font-mono-display uppercase tracking-widest text-foreground/80 flex-1">
                  {col.label}
                </span>
                <span className="text-[10px] font-mono-display text-muted-foreground/60">
                  {colTasks.length}
                </span>
              </div>
              ...
              {/* Drop zone */}
              <div
                className={`flex-1 space-y-2 overflow-y-auto scrollbar-thin rounded-lg p-1.5 transition-colors duration-150 ${
                  isOver ? `bg-secondary/40 border border-dashed ${accent.dropzone}` : "border border-transparent"
                }`}
              >
                <AnimatePresence>
                  {colTasks.map((task) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex items-start gap-1.5">
                          ...
                          <h4 className="text-[13px] font-medium text-foreground leading-snug flex-1">
                            {task.title}
                          </h4>
                        </div>
                        ...
                        {/* Description */}
                        {task.description && (
                          <p className="text-[11px] text-muted-foreground/70 line-clamp-2 mt-1.5 ml-5 leading-relaxed">
                            {task.description}
                          </p>
                        )}
                        ...
                        <div className="flex items-center justify-between mt-3 ml-5">
                          <span
                            className={`inline-flex items-center gap-1 text-[9px] font-mono-display uppercase px-1.5 py-0.5 rounded border ${priority.style}`}
                          >
                            {priority.icon}
                            {task.priority}
                          </span>
                            ...
                            {assignee ? (
                              <div className="flex items-center gap-1">
                                <img
                                  src={assignee.image}
                                  alt={assignee.name}
                                  className="h-5 w-5 rounded-full object-cover bg-secondary border border-border/50"
                                />
                                <select
                              ...
                              <div className="relative">
                                <div className="h-5 w-5 rounded-full bg-secondary/60 border border-dashed border-border/60 flex items-center justify-center">
                ...
                {/* Empty state */}
                {colTasks.length === 0 && (
                  <div className="flex items-center justify-center h-20 text-[10px] font-mono-display text-muted-foreground/40 uppercase tracking-wider">
                    Drop here
                  </div>
                )}
              </div>
